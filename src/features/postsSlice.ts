...
export const fetchPosts = createAsyncThunk("posts/fetchPost",
    async ({page, limit}: { page: number; limit: number, userId?: string }, {rejectWithValue}) => {
        try {
            const response:{data: any, meta: any} =  await apiClient.get(`/protected/posts`, {
                params: {
                    page, limit
                }
            })
...
export const fetchUsersPosts = createAsyncThunk("posts/fetchUsersPost",
    async ({page, limit, userId}: { page: number; limit: number, userId: string }, {rejectWithValue}) => {
        try {
            const response:{data: any, meta: any} = await apiClient.get(`/protected/users/posts/${userId}`, {
                params: {
                    page, limit
                }
            })
...
const postsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchUsersPosts.pending, (state) => {
            ...
            .addCase(fetchUsersPosts.fulfilled, (state, action: PayloadAction<PostsResponse>) => {
                state.status = STATUS.FULFILLED
                state.loading = false
                state.postsResponse = action.payload
                state.error = null
            ...
            .addCase(fetchUsersPosts.rejected, (state, action: any) => {
                state.status = STATUS.REJECTED
                state.error = action?.payload?.message ?? 'something went wrong'
                state.errorStatus = action?.payload?.status ?? null;
                state.loading = false
